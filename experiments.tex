...
executar o passo \emph{sfmrecon} %\ref{fig:galinhaSfM224}
, pois o MVE não
...
O software do Sense é de fácil entendimento (figura~\ref{fig:sense0}), caso necessário, pode-se traduzir para o português e possui um \emph{link} para o guia do usuário onde é explicado todo o funcionamento da ferramenta e do seu software, bem como boas práticas para ter um bom resultado na reconstrução. O programa também conta com uma boa customização (como tamanho do objeto a ser escaneado, resolução, orientação, se desejamos reconstruir as cores, entre outros parâmetros), figura~\ref{fig:sense1}. Além disso, ele faz uma rápida avaliação do computador e recomenda fazer a reconstrução a nível de GPU ou CPU, dependendo do resultado.

\begin{figure}[!h]
	\centering
	\caption{Tela inicial do Sense}
	\includegraphics[width=0.9\linewidth]{figs/boasvindassense.png}
	\legend{Quando abrimos o Sense, temos à disposição algumas informações, como dicas para um bom escaneamento e guia do usuário, por exemplo. }
		\source{
   	 O autor
   	 (2017).
   }\label{fig:sense0}
...
\begin{figure}[!h]
	...
	\includegraphics[width=0.8\linewidth]{figs/sense1.png}
	\source{
   	 O autor
   	 (2017).
   }\label{fig:sense1}
...
\begin{figure}[!h]
	\centering
	\caption{Ferramentas de edição do Sense}
	\includegraphics[width=1\linewidth]{figs/sense2.png}
	\legend{Após escanear o objeto, podemos melhorar o resultado com as ferramentas dispostas.}
		\source{
   	 O autor
   	 (2017).
   }\label{fig:sense2}
...
\begin{figure}[!h]
	\centering
	\caption{Opções de compartilhamento do Sense}
	\includegraphics[width=0.9\linewidth]{figs/sense3.png}
	\legend{Podemos exportar para alguns formatos, como \texttt{.ply}, \texttt{.obj}, entre outros.}
		\source{
   	 O autor
   	 (2017).
   }\label{fig:sense3}
...
Para comparação com os softwares que foram descritos anteriormente, fizemos o escaneamento apenas do objeto em ambiente fechado. O Sense tem dificuldades de escaneamento em áreas abertas e pode não funcionar tão bem, como descrito no \emph{site} do proprietário~\cite{3DSystems}. Para a galinha, obtivemos os resultados abaixo:
