...
%\section*{Rastreamento de curva e Aprendizado de Máquina}
%
%\begin{figure}
%  \caption {Curvas na imagem decorrentes de uma variedade de fontes físicas.}
%  \centerline{
%    \includegraphics[width=8.15cm]{figs/composition-cooking3-trim.eps}
%    \hspace{0.2cm}
%    \begin{tabular}[b]{|c|c|}
%      \hline
%      \# & Tipo de Borda \\
%      \hline
%      1 & Borda de oclusão resultante \\ & de uma superfície lisa \\ & \\
%      2 & Borda de oclusão resultante de \\ &uma descontinuidade da normal à superfície \\ & \\
%      3 & Descontinuidade da normal à superfície \\ &(cumieira afiada/\emph{sharp ridge}) \\ & \\
%      4 & Borda de descontinuidade \\ &de reflectância \\ & \\
%      5 & Borda de textura \\
%      6 & Sombras (\emph{shadows})  \\
%      7 & Curvas de tonalidade \emph{(shades)}\\
%      8 & Reflexos (\emph{highlights})\\
%      \hline
%    \end{tabular}
%  } 
%  \source{O autor, 2017.}
%  \label{fig:physical-discontinuities}
%\end{figure}
%O interesse na \textit{extração de contorno} de imagens, um problema
%fundamental e antigo em visão computacional, diminuiu no início dos anos 90 com uma mudança de
%paradigma para o uso de abordagens baseadas em pontos-chave, mas recentemente
%viu-se um avivamento, especialmente na aplicação ao reconhecimento de
%objetos~\cite{Ferrari:Fevrier:Jurie:Schmid:PAMI08,Lin:etal:CVPR09,Payet:Todorovic:ECCV10,Zhu:etal:ECCV08,Hu:Wu:Zhu:ICCV11} 
%e aplicações
%de múltiplas perspectivas~\cite{Payet:Todorovic:ICCV11,Fabbri:Kimia:CVPR10,Fabbri:Kimia:CVPR16,Fabbri:Kimia:IJCV2016,Fabbri:Giblin:Kimia:ECCV12,Nurutdinova:Fitzgibbon:ICCV2015,Kuang:Astrom:etal:ICPR2014}.
%Nesse trabalho, por \emph{contornos} ou fragmentos de \emph{curva}, queremos
%dizer explicitamente \emph{agrupamento} de elementos de \emph{borda}
%(\emph{edgels}), cuja presença de sinal de descontinuidade na intensidade ou
%informação da cor numa imagem ou nível de
%subpixel. O Capítulo~\ref{sec:cfrag:extraction} detalha essa terminologia.
%A Figura~\ref{fig:physical-discontinuities} ilustra os tipos de bordas que podem
%ser encontrados numa imagem genérica. 
%
%Nesse trabalho, foi explorada uma recente abordagem de múltiplos estágios para
%extração de curvas em
%imagens~\cite{Guo:etal:CVPR14,Guo:etal:PAMI2017:submitted}, onde o espaço de
%busca por fragmentos de curva é iterativamente reduzido removendo candidatos
%indesejados usando restrições geométricos, mas sem afetar a cobertura (sinal
%desejado), a um ponto onde a aplicação de uma objetiva funcional se torna
%apropriada.  O processo de progresso em fases de local para global: (\textit{i})
%bordas, (\textit{ii}) curvelets, (\textit{iii}) fragmentos de curva não
%ambiguos, (\textit{iv}) resolução de ambiguidades para gerar uma conjunto de
%fragmentos de candidatos de curvas, (\textit{v}) união de fragmentos de curva
%baseado em aprendizado de máquina de sinais (\emph{features}) fotométricos e geométricos
%incluindo sinais de esparsidade de bordas e (\textit{vi}) a aplicação de
%aprendizado de máquina para uma seleção final de fragmentos de curva. Detalhes
%desses estágios são discutidos nos Capítulos~\ref{sec:cfrag:extraction}
%e~\ref{sec:learning:chapter}.
%
%Os fragmentos de curva resultantes são originalmente aprendidos sem uma
%aplicação em mente, a partir de um conjunto \emph{genérico} \emph{ground-truth}
%de treinamento anotado por humanos. Eles podem ser avaliados de duas maneiras:
%
%\begin{enumerate}
%  \item Uma medida de estabilidade dos fragmentos de curva quando as imagens
%    sofrem transformações visuais como mudanças de ponto de vista, iluminação, e
%    ruído, um fator crítico para fragmentos de curva serem úteis para processos
%    visuais subsequentes, algo frequentemente ignorado em avaliações.
%  \item Uma comparação mais tradicional de anotação humana para imagens
%    genéricas, entre usar um novo conjunto de dados anotados (CFGD) ou o padrão
%    Berkeley Segmentation Dataset (BSDS), que se mostra não
%    apropriado para avaliar fragmentos de curva (esses conjuntos de dados são
%    revisados no Capítulo~\ref{sec:datasets}).
%\end{enumerate}
%
%Os resultados de ambos os métodos de avaliação são mais significativos que
%algoritmos do estado da arte em aplicações de domínios não especializados.
%Portanto, este trabalho usa esta 
%abordagem~\cite{Tamrakar:Kimia:ICCV07,Tamrakar:PHD:2008,Guo:etal:CVPR14,Guo:etal:PAMI2017:submitted}.
%O processo para imagens genéricas tem bons resultados, mas é bastante ambiguo
%pois não é específico a água ou a nenhuma aplicação. Nesse trabalho, foi explorada a construção da
%  especialização desse sistema genérico, para desafiadoras sequências de múltiplos vídeos de
%ondas de água. A especialização é feita de tal maneira que a lógica geral dos
%processos continua genérica e pode ser adaptada para outras aplicações. Isso
%efetivamente fornece um idioma para expressar o sistema (em termos de
%características e dados de treinamento), mas detalhes devem ser projetados para
%cada aplicação.
